function clickEnviar() {
    let result = document.evaluate(
        `/html/body/div[1]/div/div/div[2]/div/div/div[1]/div[1]/div[2]/section/main/div/section/div/div/div/div[1]/div[2]/div/div[1]/div/div[2]/div[2]/div/div/div/div/div/div/div/div/div/div[3]/div`,
        document,
        null,
    ...
    const enviar = result.singleNodeValue;  // pega a div encontrada
    if (enviar) {
        enviar.click();                       // clica na div
    } else {
        console.log("Elemento não encontrado");
    }
}

function escreverCampo(txt) {
    let input = document.querySelector(`div[contenteditable="true"]`)
    if (!input) throw new Error("Não há nenhum bate-papo aberto.")
    input.focus()
...
async function enviarScript(scriptText) {
    ...
    for (let linha of linhas) {
        ...
        if (linhas.indexOf(linha) !== linhas.length - 1) await new Promise(resolve => setTimeout(resolve, 250));
    }
